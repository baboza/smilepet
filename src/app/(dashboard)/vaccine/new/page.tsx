...
import { ArrowLeft, Save, Syringe, Search, ChevronDown, CalendarPlus } from "lucide-react";
...
function VaccineFormContent() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<VaccineFormValues>({
    ...
    try {
      ...
      if (data.nextAppointmentDate) {
        await addDoc(collection(db, "appointments"), {
          ...
          status: "pending",
          notes: "นัดหมายอัตโนมัติจากการบันทึกวัคซีน",
          createdAt: now.toISOString(),
          createdBy: user?.uid || "unknown"
        });
      }
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 pb-20">
      <div className="bg-white px-4 py-4 shadow-sm sticky top-0 z-30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">บันทึกวัคซีน</h1>
        </div>
      </div>
      ...
      <div className="p-4 flex-1">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-2xl mx-auto">
          ...
          <section className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            <h2 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
              <span className="bg-yellow-100 text-yellow-600 p-1.5 rounded-lg"><Syringe size={18} /></span>
              ข้อมูลวัคซีน
            </h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    {...register("weight")}
                    className="w-full p-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors"
                    placeholder="0.00"
                  />
                </div>
                <div>
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 z-40 md:relative md:bg-transparent md:border-0 md:p-0">
            <button
              ...
              className="w-full bg-yellow-500 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 hover:bg-yellow-600 active:scale-95 transition-all shadow-sm disabled:opacity-70 disabled:active:scale-100"
            >
              {isSubmitting ? (
                <>กำลังบันทึก...</>
              ) : (
...
export default function NewVaccinePage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-yellow-500"></div></div>}>
      <VaccineFormContent />
    </Suspense>
  );
}
